def gameOfNuts(nuts, comp1 = None):
	...
	while nuts > 0:																										# only runs if there is more than 1 nut on the board
		player_turn = switch_turns(player_turn)																			# switchs the player from the previous round
		nut_status(nuts)																								# print out the status of nuts
		if player_turn == 2 and comp1 is not None:																		# checks if the computer exist and it is the computers turn
			sub = comp1.makeMove(nuts)																					# the computer makes it move
			print('Player ' + str(player_turn) + ': How many nuts do you take (1-3)?', sub)
		else:
			sub = input_nuts(player_turn)																				# the player, that isn't a computer, will be asked to input a move
...
def input_nuts(player):
	...
	'''
	sub = int(input('Player ' + str(player) + ': How many nuts do you take (1-3)?'))
	if sub < 1 or sub > 3:
...
class Computer():
	...
	def __init__(self):
		self.games_played = 0																							# holds the amount of games that the AI has played
		self.table = self.generate_table()
		self.moves_made = {}																							# a dictionary that will hold the moves made by the AI. Key = nuts at the given moment. Value = move made
	...
	def makeMove(self, nuts):
		...
		'''
		row = self.table[nuts]
		rand = random.randint(1, row[1] + row[2] + row[3])																# the AI will randomly select a nut from the table, based on the statistical win/loss
		if (rand <= row[1]):
			move = 1
			...
			move = 3
		self.moves_made[nuts] = move																					# adds the move made to the dictionary
		return move
	...
	def printTable(comp1):
		...
		for row in range(len(table1) - 1, 0, -1):
			...
			print("{:^10d} {:^10d} {:^10d} {:^10d}".format(row, table1[row][1], table1[row][2], table1[row][3]))

			file.write("{:^10d} {:^10d} {:^10d} {:^10d}".format(row, table1[row][1], table1[row][2], table1[row][3]))
		file.close()
	...
	def update(self, result):
		...
		if (result == "win"):																							# if the computer has won
			for keys, value in self.moves_made.items():
				self.table[keys][value] += 1																			# it will loop through the dictionary and increase the odds of that moving being played again
		if (result == "lose"):																							# if the computer has loss
			for keys, value in self.moves_made.items():
				if self.table[keys][value] > 1:
					self.table[keys][value] -= 1																		# if will loop through the dictionary and decrease the odds of that moving being played again. Value does not drop past 1
		self.moves_made = {}																							# moves made gets cleared
		self.games_played += 1																							# AI increases the amount of games played
...
if __name__ == "__main__":
	comp1 = Computer()																									# the computer that the game will be used to train
	print('Welcome to the game of Nuts!')
	nuts = init_table() 																								# the amount of nuts that the game will be played with
	while (True):
		...
		if (user == 1):
			...
			while(playAgain()):																							# loops the game asking whether they want to play again
				gameOfNuts(nuts)

		elif (user == 2):
			gameOfNuts(nuts,comp1)																						# plays against an unskilled computer
			while (playAgain()):
				gameOfNuts(nuts,comp1)																					# loops the game whether they want to play again
		elif (user == 3):
			i = 100000
			comp2 = Computer()
			for x in range(i):
				print("[{}/{}]".format(x, i), "{:.3f}%".format((x / i) * 100))
				train(nuts,comp1,comp2)																						# trains the AI 100000 times against another computer

			gameOfNuts(nuts,comp1)																						# the user gets to play against the AI
